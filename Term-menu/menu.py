...
def launch_menu():
    ...
    curses.start_color()
    curses.init_pair(1, curses.COLOR_RED, curses.COLOR_WHITE)  # Initialize a color-pair with RED as fg and WHITE as bg
    screen.keypad(1)
    selection = -1
    option = 0
    ...
    while selection < 0:
        ...
        screen.addstr(2, 2, "Game Menu")
        screen.addstr(4, 2, "Please select an option...", curses.A_BOLD)
        screen.addstr(5, 2, "1 - Play Game", h[0])
        screen.addstr(6, 2, "2 - Instructions", h[1])
        screen.addstr(7, 2, "3 - Game Options", h[2])
        screen.addstr(8, 2, "4 - High Scores", h[3])
        screen.addstr(9, 2, "5 - Exit ('q')", h[4])
        screen.refresh()
        q = screen.getch()
